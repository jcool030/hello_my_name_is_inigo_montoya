...
public class ServiceProviderList extends ArrayList<ServiceProviderList.UserElement> implements Serializable {
    private static ServiceProviderList instance = null;
    private DatabaseHandler udbHandler;

    private ServiceProviderList(){
        super();
    }
    ...
    public class UserElement{
       ...
       public UserElement(String key, ServiceProvider user){
           this.key = key;
           this.user = user;
       }
    }

    public static ServiceProviderList getInstance(){
        if (instance == null){
            instance = new ServiceProviderList();
        }
        return instance;
    }

    public static ServiceProvider getUser(String key){
        if (instance == null){
            return null;
        } else {
    ...
    public ArrayList<ServiceProvider> getUserList(){
        ...
        udbHandler.getReferenceToUserTable().addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                for (DataSnapshot snapshot : dataSnapshot.getChildren()) {
                    ...
                    ServiceProvider temp = new ServiceProvider(username, password, firstname, lastname, type);
                    instance.add(new UserElement(snapshot.getKey(), temp));

                    if (temp == null){
                        Log.d(TAG, "user is null: " + username + " " + type);
                    }else {
                        Log.d(TAG, "Adding user from database : " + temp.toString());
                    }
                }
            }
